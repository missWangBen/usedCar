        ...
        operate: {
            ...
            removeAll: function() {
        		...
        		$.modal.confirm("确认要删除选中的" + rows.length + "条数据吗?", function() {
        			var url = $.table._option.disableUrl;
			...
			// 启用信息
			enable: function(id) {
				$.modal.confirm("确定启用该条" + $.table._option.modalName + "信息吗？", function() {
					var url = $.common.isEmpty(id) ? $.table._option.enableUrl : $.table._option.enableUrl.replace("{id}", id);
					if($.table._option.type == table_type.bootstrapTreeTable) {
						$.operate.get(url);
					} else {
						var data = { "ids": id };
			...
			enableAll: function() {
				...
				$.modal.confirm("确认要启用选中的" + rows.length + "条数据吗?", function() {
					var url = $.table._option.enableUrl;
					var data = { "ids": rows.join() };
					$.operate.submit(url, "post", "json", data);
				});
			},
			// 停用信息
			disable: function(id) {
				$.modal.confirm("确定停用该条" + $.table._option.modalName + "信息吗？", function() {
					var url = $.common.isEmpty(id) ? $.table._option.disableUrl : $.table._option.disableUrl.replace("{id}", id);
					if($.table._option.type == table_type.bootstrapTreeTable) {
						$.operate.get(url);
					} else {
						var data = { "ids": id };
			...
			disableAll: function() {
				...
				$.modal.confirm("确认要停用选中的" + rows.length + "条数据吗?", function() {
					var url = $.table._option.disableUrl;
					var data = { "ids": rows.join() };
					$.operate.submit(url, "post", "json", data);
				});
